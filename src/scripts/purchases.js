function registrarDesfazer(pa) { pilhaDesfazer.push([JSON.parse(JSON.stringify(pa))]); atualizarBotaoDesfazer(); }
    ...
    function deletarPedidoDireto(idUnico, pId, paId) { abrirConfirmacaoApp({ titulo:'Excluir pedido?', mensagem:'Este pedido será removido do histórico geral.', rotulo:'Excluir', cor:'#c62828', acao:() => { let pa = db.pedidosAtivos.find(x => x.idUnico === idUnico); if(pa) { pa.excluido = true; pa.excluidoCompras = true; pa.dataExclusao = Date.now(); pa.dataStatus = pa.dataExclusao; db.configs.syncPendente = true; salvarBanco(); sincronizarFundo(false, true); const historicoAberto = document.getElementById('modalHistoricoPedidosProduto').style.display === 'flex'; if(historicoAberto && paId && paId !== idUnico) renderizarHistoricoPedidosProduto(pId, paId); else if(paId && paId !== idUnico) abrirModalEditarPedido(paId, pId); else abrirFormProduto(pId); } } }); }
