function cancelarRascunhos() { if(confirm("Deseja realmente cancelar a seleção e remover os rascunhos?")) { db.pedidosAtivos = db.pedidosAtivos.filter(pa => pa.status !== 'rascunho'); salvarBanco(); renderizarLista(); isModalFechando = true; setTimeout(() => { isModalFechando = false; }, 400); } }
    ...
    async function enviarPedidosParaCompras() {
        ...
        try {
            ...
            const atualizado = await sincronizarFundo(true, false);
            if(!atualizado) throw new Error('Não foi possível conferir a nuvem antes do envio.');
            ...
            salvarBanco();
            document.getElementById('loadingText').textContent = 'Enviando e aguardando a confirmação...';
            isSyncingFundo = true;
            await postarBanco();
            alert('Pedido confirmado e gravado na nuvem.');
            renderizarLista();
        } catch(error) {
            ...
            salvarBanco();
            alert(`O pedido NÃO foi enviado. Os itens continuam preparados neste aparelho. ${error.message}`);
            renderizarLista();
        } finally {
