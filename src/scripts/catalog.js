function renderizarLista() {
        ...
        if (db.configs.modo === 'pedido') {
            ...
            comprasParaMostrar.forEach(pa => {
                ...
                const catObj = db.categorias.find(c => c.id === p.categoria) || { cor: '#999', corTexto: '#fff' }; let emojiStatus = pa.status === 'pendente' ? '●' : (pa.status === 'pedido_forn' ? '↗' : (pa.status === 'cancelado' ? '×' : '✓')); let isSelected = itensSelecionadosRelatorio.has(pa.idUnico); let htmlSelecao = modoSelecaoAtivo ? `<div class="checkbox-selecao"></div>` : ''; let qtyVal = pa.qtd !== '' ? pa.qtd : null; let unVal = pa.unidade ? pa.unidade : null; let qtdStrDisplay = ""; if(qtyVal !== null && unVal !== null) qtdStrDisplay = `${qtyVal} ${unVal}`; else if(qtyVal !== null) qtdStrDisplay = `${qtyVal}`; else if(unVal !== null) qtdStrDisplay = `${unVal}`; let tituloItem = escaparHtml(p.nome);
                ...
                htmlPrincipal += `<li class="item status-${pa.status} ${itemClassExtra} ${isSelected ? 'selecionado' : ''}" role="button" tabindex="0" aria-label="${escaparHtml(p.nome)}. ${nomeStatus}" data-id="${pa.idUnico}" data-grp-cat="${catGroupId}" data-grp-sub="${subcatGroupId}" onclick="cliqueItemCompra('${pa.idUnico}', this)" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault(); cliqueItemCompra('${pa.idUnico}', this)}" ontouchstart="handleTouchStart(event, this)" ontouchmove="handleTouchMove(event)" ontouchend="handleTouchEnd(event, this)">${htmlSelecao}<div style="display: flex; align-items: center; flex: 1; overflow: hidden;"><div class="status-icon ${pa.status}">${emojiStatus}</div><div class="item-info" style="overflow: hidden; text-overflow: ellipsis; padding-right: 5px;"><div class="item-title">${tituloItem}${editIndicador}</div>${obsVisual}</div></div><div class="info-direita" style="text-align: right; display: flex; flex-direction: column; align-items: flex-end; flex-shrink: 0; margin-left: 5px;">${infoDireita}</div></li>`;
                itensMostrados++;
            });
        }
    ...
    function cliqueItemCompra(idUnico, el) { if(isModalFechando || Date.now() - ultimoTouchEm < 500) return; if(modoSelecaoAtivo) { if(itensSelecionadosRelatorio.has(idUnico)) { itensSelecionadosRelatorio.delete(idUnico); el.classList.remove('selecionado'); } else { itensSelecionadosRelatorio.add(idUnico); el.classList.add('selecionado'); } } else { abrirAcoesCompra(idUnico); } }
